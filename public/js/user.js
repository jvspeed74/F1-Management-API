...
function showUsers() {
    ...
    $.ajax({
        ...
        let error = {'code': jqXHR.staus, 'status':jqXHR.responseJson.status};
...
function showUserPostsPreview(id) {
    ...
    const url = baseUrl_API + '/users/' + id + '/messages';
    ...
    $.ajax({
        ...
        displayUserPostsPreview(name, data);
    }).fail(function(xaXHR) {
        let error = {'Code': jqXHR.status, 'Status':jqXHR.responseJSON.status};
        showMessage('Error', JSON.stringify(error, null, 4));
    });
