...
function showAllPosts() {
    ...
    const url = baseUrl_API + "/messages?limit=50";
    ...
    fetch(url, {
        ...
        .then(posts => displayAllPosts(posts.data))
...
function displayAllPosts(posts, subheading=null) {
    ...
    //the row of element for adding a new message
        ...
        _html += `<div class='content-row' id='post-add-row' style='display: none'> 
            <div class='post-id post-editable' id='post-new-user_id' contenteditable='true' content="User ID">User ID</div>
            <div class='post-body post-editable' id='post-new-body' contenteditable='true'>Message Body</div>
            <div class='post-image post-editable' id='post-new-image_url' contenteditable='true'>Image</div>
...
function searchPosts() {
   console.log('searching for messages');
    ...
    fetch(url, {
        ...
        .then(checkFetch)
        .then(response => response.json())
        .then(posts => displayAllPosts(posts, subheading))
        .catch(err => showMessage("Errors", err)) //display errors
